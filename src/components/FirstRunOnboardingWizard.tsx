    ...
    <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      <div className="bg-slate-900 border border-sky-800/60 rounded-2xl max-w-2xl w-full p-5 sm:p-7 text-slate-100 shadow-2xl space-y-6 my-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Section 3: Dynamic Mac Workers Config */}
          <div className="space-y-4 bg-slate-950 p-4 rounded-xl border border-slate-800">
            <div className="flex items-center justify-between border-b border-slate-800 pb-2">
              <h3 className="text-xs font-bold text-sky-300 uppercase tracking-wider flex items-center gap-1.5">
                <span>💻</span> Динамический контур Mac Workers (WhisperX)
              </h3>

              {/* Toggle 1 or 2 Workers */}
              <div className="flex items-center bg-slate-900 p-1 rounded-lg border border-slate-700 text-xs">
                <button
                  type="button"
                  onClick={() => setActiveWorkerCount(1)}
                  className={`px-2.5 py-1 rounded font-bold transition-colors ${
                    activeWorkerCount === 1 ? 'bg-sky-600 text-white' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  1 Worker (Тест)
                </button>
                <button
                  type="button"
                  onClick={() => setActiveWorkerCount(2)}
                  className={`px-2.5 py-1 rounded font-bold transition-colors ${
                    activeWorkerCount === 2 ? 'bg-sky-600 text-white' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  2 Workers (Прод)
                </button>
            ...
            <div className="p-3 bg-slate-900 rounded-lg border border-slate-800 space-y-2">
              <span className="text-xs font-bold text-amber-300">Нода #1 (Обязательная)</span>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <input
                  type="text"
                  value={worker1Telegram}
                  onChange={(e) => setWorker1Telegram(e.target.value)}
                ...
                <input
                  type="text"
                  value={worker1Url}
                  onChange={(e) => setWorker1Url(e.target.value)}
                  placeholder="http://localhost:8000"
                  className="bg-slate-950 border border-slate-700 rounded px-2.5 py-1.5 text-xs text-amber-200 font-mono"
                />
              </div>
            </div>

            {/* Worker 2 Form (Only if activeWorkerCount === 2) */}
            {activeWorkerCount === 2 && (
              <div className="p-3 bg-slate-900 rounded-lg border border-slate-800 space-y-2 animate-fade-in">
                <span className="text-xs font-bold text-sky-300">Нода #2 (Второй ассистент)</span>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <input
                    type="text"
                    value={worker2Name}
                  ...
                  <input
                    type="text"
                    value={worker2Telegram}
                    onChange={(e) => setWorker2Telegram(e.target.value)}
                    placeholder="@igor_asst_1003"
                    className="bg-slate-950 border border-slate-700 rounded px-2.5 py-1.5 text-xs text-white font-mono"
                  />
                  <input
                    type="text"
                    value={worker2Url}
                    onChange={(e) => setWorker2Url(e.target.value)}
                    placeholder="http://localhost:8001"
                    className="bg-slate-950 border border-slate-700 rounded px-2.5 py-1.5 text-xs text-sky-200 font-mono"
                  />
                </div>
              </div>
            )}
            ...
            <div className="space-y-1 pt-1">
              <label className="text-[11px] font-semibold text-slate-300">
                WORKER_INTERNAL_SECRET (Секретный токен связи с воркерами)
              </label>
              <input
                type="text"
                value={workerSecret}
                onChange={(e) => setWorkerSecret(e.target.value)}
                placeholder="secret-worker-token-2026"
